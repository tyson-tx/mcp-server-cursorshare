...
function extractFromChatHistory(context: any): FormattedMessage[] {
  ...
  try {
    ...
    if (messages.length > 0) {
      logDebug(`从chatHistory成功提取了${messages.length}条消息`);
      return messages;
    }
  } catch (err) {
    logDebug("处理chatHistory失败:", err);
  }
...
function extractFromConversation(context: any): FormattedMessage[] {
  ...
  try {
    ...
    if (messages.length > 0) {
      logDebug(`从conversation成功提取了${messages.length}条消息`);
      return messages;
    }
  } catch (err) {
    logDebug("处理conversation失败:", err);
  }
...
function extractFromBubbles(context: any): FormattedMessage[] {
  ...
  try {
    ...
    if (messages.length > 0) {
      logDebug(`从bubbles成功提取了${messages.length}条消息`);
      return messages;
    }
  } catch (err) {
    logDebug("处理bubbles失败:", err);
  }
...
function extractFromMessagesOrHistory(context: any): FormattedMessage[] {
  ...
  try {
    ...
    if (messages.length > 0) {
      logDebug(`从messages/history成功提取了${messages.length}条消息`);
      return messages;
    }
  } catch (err) {
    logDebug("处理messages/history失败:", err);
  }
...
function extractFromArrayContext(context: any): FormattedMessage[] {
  ...
  try {
    ...
    if (messages.length > 0) {
      logDebug(`从原始数组上下文成功提取了${messages.length}条消息`);
      return messages;
    }
  } catch (err) {
    logDebug("处理原始数组上下文失败:", err);
  }
  ...
  if (context.cursorContext) {
    ...
    try {
      ...
      if (Array.isArray(context.cursorContext.messages)) {
        ...
        if (messages.length > 0) {
          logDebug(`从cursorContext.messages成功提取了${messages.length}条消息`);
          return messages;
        }
      }
      ...
      if (context.cursorContext.chat && Array.isArray(context.cursorContext.chat.messages)) {
        ...
        if (messages.length > 0) {
          logDebug(`从cursorContext.chat.messages成功提取了${messages.length}条消息`);
          return messages;
        }
      }
    } catch (err) {
      logDebug("处理cursorContext失败:", err);
    }
  }
...
export function extractConversation(context: any): FormattedMessage[] {
  ...
  logDebug("所有已知格式提取失败，尝试递归搜索对话数据");
  messages = findMessagesRecursively(context);
  
  if (messages.length === 0) {
    logDebug("无法从上下文中提取任何对话内容");
  } else {
    logDebug(`从递归搜索中找到了 ${messages.length} 条消息`);
  }
